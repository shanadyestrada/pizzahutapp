...
import com.example.pizzahutapp.model.LocalsModel
...
fun LocalPage(modifier: Modifier = Modifier) {
    ...
    var locales by remember { mutableStateOf<List<LocalsModel>>(emptyList()) }
    var selectedLocal by remember { mutableStateOf<LocalsModel?>(null) }
    ...
    LaunchedEffect(Unit) {
        ...
        db.collection("data/local/locales")
            ...
            .addOnSuccessListener { result ->
                val lista = mutableListOf<LocalsModel>()
                for (document in result) {
                    val local = document.toObject(LocalsModel::class.java)
                    lista.add(local)
                }
                locales = lista
